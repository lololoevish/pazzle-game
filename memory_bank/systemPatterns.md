...
`TownScene` использует `take_progress_update()` для сохранения переключения рычага без смены маршрута сцен.

**Применение**: хранение состояний вроде `lever_pulled` в `savegame.json` без прямого доступа сцены к файловой системе.
...
4. После первого прохождения `TownScene` позволяет опустить рычаг уровня и сохранить `lever_pulled`
5. `TownScene` перечитывает этот прогресс при новом создании и отражает статусы уровней в UI
