...
**Статус**: В текущей Rust-версии отдельный модуль сущностей удалён как неиспользуемый.

**Текущее состояние**:
- NPC как самостоятельная система в активной Rust-сборке пока не реализованы
- будущая реализация должна размещаться в отдельном модуле только при реальном использовании
